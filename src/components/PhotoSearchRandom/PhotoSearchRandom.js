...
export async function PhotoSearchRandom() {
    const searchTerm = 'random';
    const url = `https://api.unsplash.com/search/photos?query=${searchTerm}&per_page=10&client_id=${apiKeyParam}`;
    ...
    if (data && data.results && Array.isArray(data.results)) {
        ...
        console.log("entrando en photoSearch random");
